...
Для удобства использования созданы готовые батники:

- **`setup_project.bat`** - Автоматическая настройка проекта с нуля
- **`quick_test.bat`** - Быстрая диагностика системы
- **`run_pipeline.bat`** - Полный пайплайн обучения модели
- **`start_gui.bat`** - Запуск графического интерфейса
- **`start_auto_training.bat`** - Автоматическое дообучение при добавлении новых документов

## Поддержка

При возникновении проблем:
1. Запустите `quick_test.bat` для диагностики
2. Проверьте логи в папке `logs\`
3. Убедитесь в корректности входных данных
4. Проверьте наличие всех зависимостей
5. Убедитесь в валидности OpenAI API ключа